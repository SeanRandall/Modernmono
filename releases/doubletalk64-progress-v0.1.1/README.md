# DoubleTalk 64 progress build 0.1.1

> DoubleTalk 64 is the initial step in a full rewrite of the DoubleTalk
> engine. Until I have more out of DoubleTalk, we're using Monologue's rules
> to generate some words.
...
the original firmware. This compatibility build combines DoubleTalk ROM voice
and dictionary work with Monologue-derived fallback rules; it is not yet a
pure reimplementation of the DoubleTalk linguistic frontend.
...
formant frequency, tone, reverb, volume and ASCII normalization. ASCII
normalization is enabled by default so smart quotes/apostrophes, Unicode
dashes, ellipses, nonbreaking spaces and accented Latin text reach the
ASCII-only DoubleTalk frontend in a readable form. The rate control includes a
native speed boost which is kept separate from pitch; punctuation pauses also
contract as rate increases.
