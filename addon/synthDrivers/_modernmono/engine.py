...
from .phonetics import PHONEMES, parse as parse_phonetics
from .render_units import _scheduled_frames, scheduled_events
from .text_to_phonetics import RulePronouncer
from .voice_inspect import inspect_voice
    ...
    def __init__(self, voice: Path, dictionary: Path, community_dictionary: Path | None = None,
        ...
        if community_dictionary is not None and Path(community_dictionary).is_file():
            for line in Path(community_dictionary).read_text(encoding="utf-8").splitlines():
                ...
                if kind == "text":
                    # Text expansions encode deliberate casing distinctions
                    # (notably WITH, the acronym, versus ordinary "with").
